...
func packageInitClient() {
	...
	defaultClient = New(pooledClent(), token)
}
...
func (c *Client) GetSCM(project string) (string, error) {
	urlStr := c.endpoint + "/repositories/" + project

	jsonBody, err := c.get(urlStr)
	if err != nil {
...
func GetSCM(project string) (string, error) {
	// lazy load the default API client
	initOnce.Do(packageInitClient)

	return defaultClient.GetSCM(project)
}

func pooledClent() *http.Client {
	return &http.Client{
